...
```{r load data}
# loading the data and filtering the dataset for year '2016'
...
# looking into pf_ss, pf_score, and pf_expression_control
hfi_2016 %>% 
  ggplot(aes(pf_score))+
...
The third plot shows the Distribution of the Security and Safety Score, which is left skewed with a few outliers on the low end of the score.
...
1. For each pair of variables, how would you describe the relationship graphically?
  ...
  The variables pf_ss and pf_score have a linear relationship as do the variables pf_score and pf_expression_control. 
  The variables pf_ss and pf_expression_control do not have a linear relationship, the pattern is very dispersed.

2. For each pair of variables, how would you describe the relationship numerically?
    pf_score and pf_ss have a correlation of 0.753 which suggests high correlation between the two variables. pf_score and pf_espression_control have a higher correlation of 0.845, suggesting strong linear relationship between the variables.
...
3. Are your two explanatory variables collinear (correlated)?
  ...
  Yes, the two explanatory variables are collinear.
...
$$
\hat{y} = 1.4998522 + 0.3965278 \times pf_expression_control + 0.4346730\times pf_ss
$$
...
In the above code, we are creating a new variable called, 'west_atlantic' which is assigned the value of 'Yes' if the region is in North America, Latin America, and the Caribbean, else, it is assigned  'No'.
...
The information represented is the relationship between west_atlanticYes and pf_expression_control
...
\hat{y} = 4.2747325 + 0.1024089 \times west_atlanticYes + 0.5401164\times pf_expression_control
...
pf_score(yes) = 4.2747325 + 0.1024089 \times west_atlanticYes + 0.5401164\times pf_expression_control
pf(no) = 4.2747325 + 0.5401164\times pf_expression_control
